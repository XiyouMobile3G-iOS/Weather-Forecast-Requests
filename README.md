...
以下功能是本项目至少要完整跑通的最小集合，验收时业务逻辑部分以此为准：

1. **收藏夹（首页）**：展示已收藏城市的天气；收藏夹不能为空，启动时预置（写死）至少一个默认收藏城市，不依赖用户先手动添加。
2. **搜索页**：支持模糊搜索/输入联想，联想结果通过调用天气或地理编码 API 获得即可（不要求本地词库或额外的智能推荐算法）；可以查看某个搜索结果城市的天气，并将其添加到收藏。
3. **防重复收藏**：同一城市不能被重复添加到收藏夹，判重规则见下方"城市唯一性与重复添加"一节。
4. **城市详情**：展示指定城市天气的各项数据指标；具体展示哪些字段、布局如何，参考 `iOS 27 beta3.mov` 的效果即可，不要求逐像素还原，也不代表推荐的代码架构。
5. **持久化存储**：收藏城市列表能够跨启动保留属于加分项，建议实现，但不是本次验收的强制项；即使每次启动都回退到写死的默认收藏城市，也不影响通过。
6. **图标**：Tab、按钮等系统性图标可以优先使用 SF Symbols（Objective-C 中可用 `[UIImage systemImageNamed:]` 获取），不强制要求自绘图标资源。
...
5. 业务逻辑：实际追踪“启动 → 模糊搜索城市 → 选择城市 → 展示天气 → 添加 → 防重复 → 查看 → 删除”的流程。搜索可使用 UISearchBar，也可使用 UITextField + UITableView 自行实现，不得仅因控件选择不同而扣分；重点检查模糊查询行为。搜索应有合理防抖；空输入立即清空；数据源必须先更新、TableView 后在主线程刷新；旧请求不得覆盖新关键词结果。城市应以稳定 ID 为唯一 Key，无 ID 时使用地区与经纬度组合，不能只用展示名称；判重与写入必须是同一串行操作，连续点击也只能添加一次，删除时同步清理 Key。另检查加载、空数据、无网络、超时、非 2xx、无效 JSON、字段缺失、数组越界等情况是否有合理反馈且不崩溃。收藏夹不能为空：应用启动时必须已有至少一个预置（写死）的默认收藏城市，不依赖用户先手动添加。收藏的持久化存储（跨启动保留）不是强制项，未做持久化、每次启动都回退到默认收藏城市，不应作为不通过的理由；但若声称做了持久化却实际无效，仍应指出。
